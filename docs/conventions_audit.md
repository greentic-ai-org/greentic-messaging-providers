...
- Wiring:
  ...
  - CI test: provider workflows and `crates/greentic-messaging-packgen/tests/packgen.rs`.
...
- CI tests: provider workflows and `crates/greentic-messaging-packgen/tests/packgen.rs`.
...
### GitHub Actions
- provider build/release workflows:
...
- `.github/workflows/nightly-provider-e2e.yml`:
  - builds the selected provider pack locally or pulls a published `.gtpack`
  - runs provider e2e via `ci/e2e/provider_e2e.py`
  - uploads sanitized per-provider result artifacts
...
- Add `ci/gen_flows.sh` before flow validation in the provider build workflows.
