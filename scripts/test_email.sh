#!/usr/bin/env bash
# Start a local SMTP email tester UI.
#
# Usage:
#   scripts/test_email.sh [--port <port>] [--no-build] [--no-open]
...
if [ "${BUILD}" -eq 1 ]; then
  PACK_VERSION="$(python3 -c "import json; print(json.load(open('ci/provider-matrix.json'))['providers']['email']['version'])")"
  bash tools/build_components/messaging-provider-email.sh
  PACK_FILTER=messaging-email PACK_VERSION="${PACK_VERSION}" ./ci/steps/11_build_packs.sh
...
export GREENTIC_EMAIL_TEST_PROVIDER="email"
export GREENTIC_EMAIL_TEST_TITLE="SMTP Email"
