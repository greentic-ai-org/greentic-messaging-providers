...
- `messaging.teams.graph` (capabilities: messaging; ops: send, reply, render_plan, encode, send_payload, qa-spec, apply-answers, i18n-keys)

## Components

- `messaging-provider-teams-graph` - core Graph provider WASM (secrets-store + http-client)
...
Azure Bot Service, redirect OAuth callbacks, and client secrets are not required for this Graph path. `public_base_url` is only needed for Graph change-notification ingress.

This pack does not advertise Bot Framework interactivity. Adaptive Card submit
callbacks belong in a dedicated Teams bot/Bot Framework provider.

## Extensions

- `greentic.ext.capabilities.v1` - capability offer `messaging-teams-graph-v1`
