# Messaging Provider Teams Graph Component

Provider-core Microsoft Teams messaging provider for Microsoft Graph egress and
Graph change-notification read ingress.
...
`secrets://{env}/{tenant}/_/messaging-teams-graph/` where `{env}` must match
...
The setup path uses Microsoft OAuth device-code login against the
...
This provider does not promise Bot Framework interactivity. Adaptive Card
submit callbacks require a dedicated Teams bot/Bot Framework provider.
