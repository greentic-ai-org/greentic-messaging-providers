...
mod tests {
    ...
    #[test]
    fn send_payload_returns_send_error_for_missing_destination_without_network() {
        let input = send_payload_input(PROVIDER_TYPE, STANDARD.encode(br#"{"body":"hi"}"#));
        let body = parse_result(send_payload(&serde_json::to_vec(&input).expect("input")));

        assert_eq!(body["ok"], false);
        assert_eq!(body["message"], "destination required");
        assert_eq!(body["retryable"], false);
    }

    #[test]
    fn send_payload_returns_missing_secret_error_without_network() {
        let input = send_payload_input(
            PROVIDER_TYPE,
            STANDARD.encode(json!({"to": "+15551230001", "body": "hi"}).to_string()),
        );
        let body = parse_result(send_payload(&serde_json::to_vec(&input).expect("input")));
